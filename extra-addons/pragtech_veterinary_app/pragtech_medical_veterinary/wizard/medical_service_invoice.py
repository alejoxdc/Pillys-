...
class medical_service_invoice_wizard(models.TransientModel):
    _inherit = 'medical.service.invoice.wizard'
    ...
    def create_invoice(self):
        ...
        for service in services:
            ...
            curr_id = service.patient.name.company_id.currency_id.id
            if service.patient.name.is_pet:
                partner_id = service.patient.name.owner_name.id
                acc_id = service.patient.name.owner_name.property_account_receivable_id.id
                curr_id = service.patient.name.owner_name.company_id.currency_id.id
            if invoice_ex == False:
                ...
                for line in service.service_line:
                    if line.to_invoice:
                        ...
                        service.write({'inv_id': invoice_id.id, 'state': 'invoiced'})
        return {
            'name': 'Create Invoice',
            'view_mode': 'tree,form',
