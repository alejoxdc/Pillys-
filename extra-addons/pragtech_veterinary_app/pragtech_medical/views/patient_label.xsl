...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
    ...
    <xsl:template match="lots">
        <document>
            <template leftMargin="2.0cm" rightMargin="2.0cm" topMargin="2.0cm" bottomMargin="2.0cm" title="Address list"
                      author="Generated by Open ERP">
                <pageTemplate id="all">
                    <pageGraphics/>
        ...
        <xsl:if test="position() &lt; $max_frames + 1">
            <frame>
                ...
                <xsl:attribute name="y1">
                    <xsl:value-of
                            select="$initial_bottom_pos - floor((position()-1) div $number_columns) * $height_increment"/>
                    <xsl:text>cm</xsl:text>
                </xsl:attribute>
            </frame>
        </xsl:if>
    ...
    <xsl:template match="lot-line" mode="story">
        <blockTable style="mytable" colWidths="2.8cm,5.4cm">
            ...
            <tr>
                ...
                <td>
                    <para style="nospace">Name :
                        <xsl:choose>
                            <xsl:when test="patientlastname != ''">
                    ...
                    <xsl:text>,</xsl:text>
                    <para style="nospace">DOB :
                        <xsl:value-of select="dob"/>
                    </para>
                    <para style="nospace">QID :
                        <xsl:value-of select="patientref"/>
                    </para>
                    <para style="nospace">Blood Group :
