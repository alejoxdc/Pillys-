...
class Invoice(models.Model):
    ...
    def generar_invoice_tax(self,document=None):
        ...
        if self.currency_id.name != 'COP':
            calculation_rate = self.current_exchange_rate or 1
        else:
            calculation_rate = 1
        ...
        for line_id in self.invoice_line_ids.filtered(lambda line: line.display_type == 'product'):
            for tax in line_id.tax_ids:
                ...
                if '-' not in str(tax.amount) and tax.tributes != 'ZZ':
                    ...
                    if line_id.price_subtotal != 0:
                        price_subtotal_calc = line_id.price_subtotal * calculation_rate
                    else:
                        taxes = False
                        ...
                        price_subtotal_calc = (taxes['total_excluded'] if taxes else line_id.quantity * line_id.line_price_reference) * calculation_rate
                ...
                #retenciones
                    ...
                    if tax.tributes != 'ZZ':
                        ...
                        if line_id.price_subtotal != 0:
                            ...
                            if line_id.tax_line_id and line_id.tax_line_id != 'ZZ':
                                taxes = line_id.tax_line_id.compute_all(line_id.line_price_reference, line_id.currency_id, line_id.quantity,product=line_id.product_id,partner=self.partner_id)
                            price_subtotal_calc = taxes['total_excluded'] if taxes else line_id.quantity * line_id.line_price_reference * self.current_exchange_rate or 1
        ...
        for tax_type, ret in ret_total_values.items():
            if self.currency_id.name != 'COP':
                if tax_type == '05':
                    rete_cop['rete_iva_cop'] = abs(ret['total'])
                elif tax_type == '06':
                    rete_cop['rete_fue_cop'] = abs(ret['total'])
                elif tax_type == '07':
                    rete_cop['rete_ica_cop'] = abs(ret['total'])
        contador = 1
        total_impuestos=0
        for index, invoice_line_id in enumerate(self.invoice_line_ids.filtered(lambda line: line.product_id)):
            if invoice_line_id.price_unit>=0:
                if invoice_line_id.price_subtotal != 0:
                    price_subtotal_calc = invoice_line_id.price_subtotal * calculation_rate
                else:
                    taxes = False
                    ...
                    if '-' not in str(tax.amount) and tax.tributes != 'ZZ':
                        ...
                        if invoice_line_id.price_subtotal != 0:
                            ...
                            if invoice_line_id.tax_line_id:
                                taxes = invoice_line_id.tax_line_id.compute_all(invoice_line_id.line_price_reference, invoice_line_id.currency_id, invoice_line_id.quantity,product=invoice_line_id.product_id,partner=self.partner_id)
                            price_subtotal_calc = taxes['total_excluded'] if taxes else invoice_line_id.quantity * invoice_line_id.line_price_reference * calculation_rate
                        ...
                        if tax.amount not in tax_info[tax.codigo_dian]['info']:
                            ...
                            aux_total = round(aux_total, 2)
                            aux_taxable = round(aux_taxable, 2)
                            tax_info[tax.codigo_dian]['info'][tax.amount]['value'] = aux_tax #* self.current_exchange_rate
                            tax_info[tax.codigo_dian]['total'] = aux_total #* self.current_exchange_rate
                            tax_info[tax.codigo_dian]['info'][tax.amount]['taxable_amount'] = aux_taxable #* self.current_exchange_rate
                    else:
                        if tax.tributes == '06':
                ...
                if invoice_line_id.discount:
                    ...
                    discount_line = round(discount_line * self.current_exchange_rate, 2)
                    discount_percentage = invoice_line_id.discount
                    base_discount = invoice_line_id.price_unit * invoice_line_id.quantity * self.current_exchange_rate or 1
                else:
                    discount_line = 0
                ...
                if not invoice_line_id.product_id.enable_charges:
                    ...
                    mapa_line={
                        ...
                        'line_extension_amount': invoice_line_id.price_subtotal * self.current_exchange_rate or 1,
                        'item_description': saxutils.escape(invoice_line_id.name),
                        'price': ((invoice_line_id.price_subtotal + discount_line)/ invoice_line_id.quantity) * self.current_exchange_rate or 1,
                        ...
                        'line_trade_sample_price': invoice_line_id.line_trade_sample_price * self.current_exchange_rate or 1,
                        'line_price_reference': (invoice_line_id.line_price_reference*invoice_line_id.quantity) * self.current_exchange_rate or 1,
                        ...
                        for item in invoice_line_id.tax_ids:
                            if not item.amount < 0:
                                ...
                                if taxs > 1:  # si hay mas de un impuesto no se incluye  a la suma del tax_exclusive_amount
                                    ...
                                    if line_id.price_subtotal != 0:
                                        tax_exclusive_amount += invoice_line_id.price_subtotal * self.current_exchange_rate or 1 
                                    else:
                                        taxes = False
                                        if invoice_line_id.tax_line_id and line_id.tax_line_id != 'ZZ':
                                            taxes = invoice_line_id.tax_line_id.compute_all(invoice_line_id.line_price_reference, invoice_line_id.currency_id, invoice_line_id.quantity,product=invoice_line_id.product_id,partner=self.partner_id)
                                        price_subtotal_calc = taxes['total_excluded'] if taxes else invoice_line_id.quantity * invoice_line_id.line_price_reference 
                                        tax_exclusive_amount += (price_subtotal_calc * self.current_exchange_rate or 1  ) 
            else:
                contador -= 1
            #fin for
        ...
        if self.move_type in ["out_invoice", "out_refund"]:
            cufe_cuds,qr,cude_seed,qr_code = self.calcular_cufe(tax_total_values, abs(invoice.amount_untaxed_signed), total_impuestos)
        if self.move_type in ["in_invoice", "in_refund"]:
            cufe_cuds,qr,cude_seed,qr_code = self.calcular_cuds(tax_total_values,abs(invoice.amount_untaxed_signed), total_impuestos)       
        tax_xml = self.generate_tax_xml(tax_total_values,self.company_id.currency_id.name)
        ret_xml = self.generate_ret_xml(ret_total_values,self.company_id.currency_id.name)
        line = self.create_invoice_lines(invoice_lines,self.company_id.currency_id.name)
        tax_cop = {'tot_iva_cop': 0.0, 'tot_inc_cop': 0.0, 'tot_bol_cop': 0.0, 'imp_otro_cop': 0.0}
        if len(tax_total_values) > 0 and (self.currency_id.name != 'COP'):
            for tax_type, ret_total in tax_total_values.items():
                if tax_type == '01':
                    tax_cop['tot_iva_cop'] = ret_total['total']
                elif tax_type == '04':
                    tax_cop['tot_inc_cop'] = ret_total['total']
                elif tax_type == '22':
                    tax_cop['tot_bol_cop'] = ret_total['total']
                else:
                    tax_cop['imp_otro_cop'] += ret_total['total']
        return {
                'cufe': cufe_cuds,
                'cude_seed': cude_seed,
                'qr':qr,
                ...
                'line': line,
                'line_extension_amount': '{:.2f}'.format(abs(invoice.amount_untaxed_signed)),
                'tax_inclusive_amount': '{:.2f}'.format(abs(invoice.amount_untaxed_signed) + total_impuestos),
                'tax_exclusive_amount': '{:.2f}'.format(tax_exclusive_amount),
                'payable_amount': '{:.2f}'.format(abs(invoice.amount_untaxed_signed) + total_impuestos), #invoice.amount_total + invoice.total_withholding_amount),
                ...
                'rete_fue_cop': rete_cop.get('rete_fue_cop'),
                'rete_iva_cop': rete_cop.get('rete_iva_cop'),
                'rete_ica_cop': rete_cop.get('rete_ica_cop'),
                'tot_iva_cop': tax_cop.get('tot_iva_cop'),
                'tot_inc_cop': tax_cop.get('tot_inc_cop'),
                'tot_bol_cop': tax_cop.get('tot_bol_cop'),
                'imp_otro_cop': tax_cop.get('imp_otro_cop'),
    ...
    def create_invoice_lines(self, invoice_lines, currency_id):
        ...
        for invoice_line in invoice_lines:
            ...
            ET.SubElement(price, 'cbc:PriceAmount', {'currencyID': currency_id}).text = '%0.2f' % float(invoice_line['price'])
            if invoice_line.get('uom_product_id'):
                ET.SubElement(price, 'cbc:BaseQuantity', {'unitCode': "EA"}).text = '%0.2f' % float(invoice_line['invoiced_quantity']) #{'unitCode': invoice_line['uom_product_id'].name}).text = str(invoice_line['invoiced_quantity'])
            else:
                ET.SubElement(price, 'cbc:BaseQuantity', {'unitCode': "EA"}).text = '%0.2f' % float(invoice_line['invoiced_quantity'])
