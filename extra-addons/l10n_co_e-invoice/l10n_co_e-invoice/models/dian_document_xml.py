...
class DianDocument(models.Model):
    ...
    def _create_xml_invoice(self, template_data):
        ...
        ET.SubElement(cac_Delivery_address, 'cbc:ID').text = str(self._replace_character_especial(self.document_id.partner_shipping_id.city_id.code))
        ET.SubElement(cac_Delivery_address, 'cbc:CityName').text = str(self._replace_character_especial(self.document_id.partner_shipping_id.city_id.name))
        ET.SubElement(cac_Delivery_address, 'cbc:CountrySubentity').text = str(self._replace_character_especial(self.document_id.partner_shipping_id.city_id.state_id.name))
        ET.SubElement(cac_Delivery_address, 'cbc:CountrySubentityCode').text = str(self._replace_character_especial(self.document_id.partner_shipping_id.city_id.state_id.code_dian))
        cac_delivery_address_line = ET.SubElement(cac_Delivery_address, 'cac:AddressLine')
        ET.SubElement(cac_delivery_address_line, 'cbc:Line').text =  str(self._replace_character_especial(self.document_id.partner_shipping_id.street))
        ...
        ET.SubElement(cac_legal_monetary_total, 'cbc:AllowanceTotalAmount', attrib={'currencyID': str(template_data['CurrencyID'])}).text = '{:.2f}'.format(self.document_id.invoice_discount)
        ET.SubElement(cac_legal_monetary_total, 'cbc:ChargeTotalAmount', attrib={'currencyID': str(template_data['CurrencyID'])}).text = '{:.2f}'.format(0.00)
    ...
    def _create_ds_xml(self,data):
        ...
        ET.SubElement(cac_party_tax_scheme, 'cbc:CompanyID', {
            'schemeAgencyID': '195',
            'schemeAgencyName': 'CO, DIAN (Dirección de Impuestos y Aduanas Nacionales)',
    ...
    def _create_xml_nc(self,template_data):
        ...
        ET.SubElement(DiscrepancyResponse, 'cbc:ResponseCode').text =  str(template_data['DiscrepancyResponseCode'])
        ET.SubElement(DiscrepancyResponse, 'cbc:Description').text =  str(template_data['DiscrepancyResponseDescription'])
        ...
        ET.SubElement(PartyTaxScheme, 'cbc:CompanyID', {'schemeAgencyID': '195',
                                                        'schemeAgencyName': 'CO, DIAN (Dirección de Impuestos y Aduanas Nacionales)',
                                                        'schemeID': str(template_data['CustomerschemeID']),
                                                        'schemeName': '31'}).text = str(template_data['CustomerID'])
    ...
    def _generate_data_invoice_document_xml(self, dc, dcd, data_taxs_xml,  data_lines_xml,  CUFE):
        data = {
            ...
            "ResponseCodeCreditNote": dcd.get("esponseCodeCreditNote", ""),
