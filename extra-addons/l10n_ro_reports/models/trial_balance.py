...
class L10nRoTrialBalance5ColumnReportHandler(models.AbstractModel):
    ...
    def _create_column_group_total(self, report, options, previous_options, default_group_vals, side_to_append):
        ...
        forced_options = {'date': initial_forced_options['date']}
        forced_options['date']['date_to'] = options['date']['date_to']
    ...
    def _custom_line_postprocessor(self, report, options, lines, warnings=None):
        ...
        for line in lines:
            ...
            for col in report.column_ids:
                sub_column_sum = sum(x['no_format']
                                     for x in line['columns']
                                     if x['column_group_key'] not in sum_excluded_column_group_keys and x['expression_label'] == col.expression_label)

                total_sub_column = next(x for x in total_columns if x['expression_label'] == col.expression_label)
                total_sub_column['no_format'] = sub_column_sum
                total_sub_column['name'] = report._format_value(
                    options,
                    total_sub_column['no_format'],
                    total_sub_column['currency'],
                    blank_if_zero=total_sub_column['blank_if_zero'],
                    figure_type=total_sub_column['figure_type'],
                )
